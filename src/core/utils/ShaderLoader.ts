...
export class ShaderLoader {
   ...
   * 加载顶点着色器
   */
  static async loadVertexShader(url: string): Promise<string> {
    const response = await fetch(url)
    if (!response.ok) {
      throw new Error(`Failed to load vertex shader: ${url}`)
    }
   ...
   * 加载片段着色器
   */
  static async loadFragmentShader(url: string): Promise<string> {
    const response = await fetch(url)
    if (!response.ok) {
      throw new Error(`Failed to load fragment shader: ${url}`)
    }
    return await response.text()
  }
   ...
   * 加载完整的着色器材质
   */
  static async loadShaderMaterial(
    vertexUrl: string,
    fragmentUrl: string,
    uniforms: Record<string, THREE.IUniform> = {}
  ): Promise<THREE.ShaderMaterial> {
    const [vertexShader, fragmentShader] = await Promise.all([
      this.loadVertexShader(vertexUrl),
      this.loadFragmentShader(fragmentUrl)
    ])

    return this.createShaderMaterial(vertexShader, fragmentShader, uniforms)
  }

  /**
   * 创建 CoffeeSteam 着色器材质
   */
